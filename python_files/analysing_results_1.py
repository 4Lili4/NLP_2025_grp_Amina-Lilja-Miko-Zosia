...
def inter_annotator(results_csv):
    ...
    return results, annotator1_results, annotator2_results


def silver_data_metrics(gold_a, gold_b, silver, results):
    
    if not (len(silver) == len(gold_a) == len(gold_b)):
    ...
    accuracy = calculate_accuracy2(silver, gold_a)
    ...
    for index, row in results.iterrows():
        ...
        if silver_data_index >= len(silver):
            break
    
        silver_label = silver[silver_data_index]
